      ...
      <main className="pl-64 pt-16 min-h-screen">
        <div className="p-8 max-w-[1600px] mx-auto">
          ...
          <section className="mt-4">
            ...
            <div className="space-y-4">
              {/* First table — includes the bank-name header. Subsequent
                  tables are headerless so the names only appear once. All
                  three use table-fixed + identical colgroup widths so the
                  rate columns line up vertically across the gaps. */}
              <div className="bg-white rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.04)] overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse table-fixed">
                    <RateColGroup bankCount={banks.length} />
                    <thead>
                    ...
                    <RateRows banks={banks} terms={MAIN_TERMS} mode={rateMode} />
                  </table>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.04)] overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse table-fixed">
                    <RateColGroup bankCount={banks.length} />
                    <RateRows banks={banks} terms={LONG_TERMS} mode={rateMode} />
                  </table>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.04)] overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse table-fixed">
                    <RateColGroup bankCount={banks.length} />
                    <RateRows banks={banks} terms={FLOATING_TERM} mode={rateMode} />
                  </table>
                </div>
              </div>
            </div>
...
/**
 * Shared column widths applied to every rate sub-table so values line
 * up vertically across the three tables (main-terms, long-terms,
 * floating). Term column fixed at 140px; remaining width split equally
 * between the bank columns.
 */
function RateColGroup({ bankCount }: { bankCount: number }) {
  return (
    <colgroup>
      <col style={{ width: '140px' }} />
      {Array.from({ length: bankCount }).map((_, i) => (
        <col key={i} style={{ width: `calc((100% - 140px) / ${bankCount})` }} />
      ))}
    </colgroup>
  );
}
...
/**
 * Shared tbody markup for every rate sub-table. Keeps zebra striping and
 * hover behaviour consistent whether the table has a header or not.
 */
function RateRows({
  banks, terms, mode,
}: {
